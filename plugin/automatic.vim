...
if exists("s:rtltree_init_max_display_layer") == 0
    let s:rtltree_init_max_display_layer = 3
endif
...
map <C-F2>                                                              :call AutoDef()<ESC>
map <C-F3>                                                              :call AutoInst(0)<ESC>
...
function ShowCall() "{{{2
    ...
    if(s:rtl_tree_is_open)
        ...
        let tmp_word = expand("<cword>")

        "call extend(s:tree_array,[s:current_node])
        "let s:tree_array_idx = s:tree_array_idx + 1

        let s:current_node = s:current_node.parent
        call s:GotoInstFile(s:current_node.instname,'','^\s*`\?' . node.instname . '\s\+\(#.*\)\?' . node.instname2. '\s*(')
        "add a `\? char for instname is macro define

        call search('\.' . tmp_word)
        normal zz
    endif
endfunction "}}}2
...
function ShowDef() "{{{2
	...
	let line = getline(lnum)
	if line =~ '\<autoinst\>'
		"echohl ErrorMsg | echo "========ShowDef() ok0=========" | echohl None
        if(s:rtl_tree_is_open)
            "call extend(s:tree_array,[s:current_node])
            "let s:tree_array_idx = s:tree_array_idx + 1

            let s:current_node = s:oTreeNode.SearchChildNodeByInstname2(s:current_node,s:GetInstName2(line))
        endif

        "add for autoinst oneline mode
        let portname = ''
        let word = expand("<cword>")

        let ret = match(line,'\.' . word)
        if ret != -1
            let portname = word
        else
            let portname = matchstr(line,'\.\w\+\s*(\s*'.word)
            let portname = matchstr(portname,'\w\+')
        endif
        call s:GotoInstFile(s:GetInstName(line),portname,'')
        return
	endif

	if line !~ '^\s*\.\w'
		echohl ErrorMsg | echo "========ShowDef() fail cause not .inst_port line=========" | echohl None
		return
	else
		"echohl ErrorMsg | echo "========ShowDef() ok1=========" | echohl None
        if(s:rtl_tree_is_open)
            "call extend(s:tree_array,[s:current_node])
            "let s:tree_array_idx = s:tree_array_idx + 1

            let s:current_node = s:oTreeNode.SearchChildNodeByInstname2(s:current_node,s:GetInstName2(getline(s:GetInstLineNum(lnum))))
        endif

		call s:GotoInstFile(s:GetInstName(getline(s:GetInstLineNum(lnum))),s:GetPortName(line),'')
	endif

endfunction "}}}2

function s:GotoInstFile(inst,port,inst2) "{{{2
	if s:GetInstFileName(a:inst) == ''
	...
	if a:port != ''
		...
		call search('^\s*'. s:VlogTypePorts . '\s*' . s:VlogTypeDatas . '*' . '\s*' . '\(\[.*:.*\]\)*' . '\s*' . a:port)
		call search('\<' . a:port . '\>')
    elseif a:inst2 != ''
        call search(a:inst2)
	endif
...
function s:SkipCommentLine2(mode,lines,line_idx) "{{{2
	"mode: 0 - code search dir is up to down, 1 - is down to up
	"return no comment line index
	"must run before comment line
	let in_comment_pair = 0
...
function s:SkipPairCommentLine(mode,lines,line_idx) "{{{2
	"mode: 0 - code search dir is up to down, 1 - is down to up
	"return no comment line index
	"must run before comment line
	let in_comment_pair = 0
...
" key is seq 
	...
	while line_index <= len(a:lines)
		"1 step,skip comment
		"let line_index = s:SkipCommentLine2(0,a:lines,line_index)
		let line_index = s:SkipPairCommentLine(0,a:lines,line_index)
		if line_index == -1
			break
		endif
		"2 step get define
...
function KillAutoArg() "{{{2
   ...
   while line_index <= line("$") 
      let line = getline(line_index)
      if line =~ '^\s*\<module\>\s\+\w\+\s*(/\*\<\(autoarg\|AUTOARG\)\>\*/'
...
function s:setupTreeSyntaxHighlighting() "{{{2
    ...
    syn match treeFile  #|-.*# contains=treeLink,treePart,treeRO,treePartFile,treeBookmark,treeExecFile,treeUnresolved
...
function s:bindMappings() "{{{2
    nnoremap <buffer> <cr> :call <SID>active()<cr>
    nnoremap <buffer> <leftrelease> :call <SID>checkactive(0)<cr>
    nnoremap <buffer> <2-leftmouse> :call <SID>active()<cr>
endfunction "}}}2
function s:checkactive(mode) "{{{2
    "mode 0 - for check, 1 - for active
    let lnum = line(".")
    if lnum == 1
        return
    endif
    ...
    let s:tmp_node = s:oTreeNode.SearchNodeByLnum(s:rtltree,lnum)

    let line = getline(lnum)
    "for check
    if((line[col(".")-1] =~ '+') && (a:mode == 0)) || ((line =~ '+\w') && (a:mode == 1))
        let s:tmp_node.isFold = 0

        "create children node
        for node in s:tmp_node.children
            if node.childrensolved == 0
                call s:oTreeNode.CreateRtlTree(node)
            endif
        endfor

        call s:oTreeNode.RenderTree()
    "elseif ((line[col(".")-1] =~ '\~') && (a:mode == 0)) || ((line =~ '\~\w') && (a:mode == 1))
    elseif ((line[col(".")-1] =~ '\~') && (a:mode == 0))
        let s:tmp_node.isFold = 1
        call s:oTreeNode.RenderTree()
    endif
...
function s:active() "{{{2
    ...
    let s:current_node = s:checkactive(1)

    "wincmd p
    execute bufwinnr(t:RtlBufName) . " wincmd w"

    let s:GotoInstFile_use = 1
	"if s:GetInstFileName(inst) == ''
    if s:current_node.unresolved == 1
		"echohl ErrorMsg | echo "file: tags not exist or need update!" | echohl None
		"echo ""
		"return
        let node = s:current_node
